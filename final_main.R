...
if (length(args)==0){
  source('functions.R')
  mechanistic_mod = 'cubic'
  clinic_mod = 'em'
  # parameters for clinic model
  local = T
  dirr = "/Users/xuehao/Dropbox/harvard/emvs/code/results/"
}else{
  # source('/n/data1/hsph/biostat/celehs/lab/jl762/haoxue/emvs/functions.R')
  source('/home/hx222/emvs/functions.R')
  mechanistic_mod = args[1]
  clinic_mod = args[2]
  local = F
  # dirr = "/n/data1/hsph/biostat/celehs/lab/jl762/haoxue/emvs/"
  dirr = "/home/hx222/emvs/"
}

if (local==T){
  parentPath = "/Users/xuehao/Dropbox/harvard/emvs/code/results/"
  if (mechanistic_mod=='cubic'){
    data_path = paste0(parentPath,"cubic/Mcubic.RData")
  }else if (mechanistic_mod=='quadratic'){
    data_path = paste0(parentPath,"quadratic/Mquadratic.RData")
  }else{
    data_path = "~/Dropbox/harvard/emvs/code/results/gbm/0929.RData"
  }
}else{
  # parentPath = "/n/data1/hsph/biostat/celehs/lab/jl762/haoxue/emvs/"
  ...
  if (mechanistic_mod=='cubic'){
    ...
    data_path = paste0(parentPath,"linear/linear.RData")
  }
  #functionalClassification <- read.delim("/home/hx57/emvs/functionalClassification/functionalClassification.txt", header=FALSE, comment.char="#")
}
...
# GBM_data = get_GBM_data(mechanistic_mod=mechanistic_mod)
# G = GBM_data$G
# C = GBM_data$C
# Y = log(GBM_data$Y)
# M = GBM_data$M
...
# ################################################################################
# #first-stage
GenDat=data.frame(cbind(Y,G))

# Making Grouping matrix Zmatrix
gene_group = get_grouping()
# run emvs algorithm
EMVS_res = EMVS(M,G,grouping=gene_group,nu0=0.5,nu1=10^3,nu=1,lambda=1,a=1,b=1)
# 
save(G,C,M,Y,Delta,GenDat,EMVS_res,file=paste0(parentPath,mechanistic_mod,
                          '/','EMVS_sub.Rdata'))
load(paste0(parentPath,mechanistic_mod,"/EMVS_sub.Rdata"))
N=nrow(G)
K=ncol(G)
L = K-K0
J=ncol(M)
...
mypath = paste0(dirr,mechanistic_mod,'/',clinic_mod,'/')
...
Zmatrix=cbind(matrix(1,nrow=K0,ncol=1),matrix(0,nrow=K0,ncol=3))
for(ww in 1:K0){
...
# gs = c(1)
# as = c(.1)
# final_list = rep(list(rep(list(rep(list(1),n_fold)),length(gs))),length(as))
final_list = list()
print("a0")
print(a0)
print('g')
...
for (jjj in 1:n_fold){
  test_indx = folds[[jjj]]
  train = GenDat[-test_indx,]
  test = GenDat[test_indx,]
  train_delta = Delta[-test_indx]
  test_delta = Delta[test_indx]
  if (clinic_mod=='mcmc'){
    lst = NEG_mcmc(Y=train[,1],G=as.matrix(train[,2:(K0+1)]),C=as.matrix(train[,(K0+2):(K+1)]),a0=a0,gstr=gstr,Zmatrix=Zmatrix,mypath=mypath)
  }else if (clinic_mod=='em'){
    lst = NEG_fun(Y=train[,1],G=as.matrix(train[,2:(K0+1)]),C=as.matrix(train[,(K0+2):(K+1)]),a0=a0,gstr=gstr,Zmatrix=Zmatrix,mypath=mypath)
...
ppp=1
for (gstr in c(1)){
  for (a0 in c(2,5,10,100)){
    fpath = paste0(mypath,"10fold_",
                   a0,"_",gstr,"_",clinic_mod,"_sub.RData")
    load(fpath)
